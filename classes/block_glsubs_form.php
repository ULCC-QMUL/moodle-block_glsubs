...
class block_glsubs_form extends moodleform
{
    ...
    public function definition()
    {
        ...
        $counter_record = $DB->get_record_sql('SELECT count( DISTINCT id ) AS entries  FROM {glossary_entries} WHERE glossaryid = :glossaryid AND approved = 1 AND id NOT IN (SELECT entryid FROM {glossary_entries_categories} ) ', array('glossaryid' => $glossaryid));
    ...
    protected function get_user_subscriptions()
    {
        ...
        $sqlstmt = 'SELECT itemid, count(itemid) AS counter FROM {comments} AS c 
JOIN {glossary_entries} AS ge ON c.itemid = ge.id AND ge.glossaryid = :glossaryid 
WHERE commentarea = :commentarea 
GROUP BY itemid ORDER BY itemid';
        $this->usersubscriptions->conceptCounters = $DB->get_records_sql($sqlstmt, array('glossaryid' => $glossaryid, 'commentarea' => 'glossary_entry'));
