...
class find_subscribers extends \core\task\scheduled_task
{
    ...
    protected function find_full_subscriptions($timenow)
    {
        ...
        $sql = ' SELECT min(id) AS logid, glossaryid, timecreated, 0 AS full FROM {block_glsubs_event_subs_log} 
...
GROUP BY userid,glossaryid,eventlink, timecreated ORDER BY timecreated,glossaryid';
    ...
    protected function find_new_uncategorised_subscriptions($timenow)
    {
        ...
        $sql = "
SELECT $row_id AS i , l.id AS logid , f.userid FROM {block_glsubs_event_subs_log} AS l 
JOIN {block_glsubs_glossaries_subs} AS f ON f.glossaryid = l.glossaryid AND f.active = 0 AND f.newentriesuncategorised = 1
WHERE l.processed = 0 AND l.timecreated < :timenow  AND l.eventtype = :eventtype AND ( l.conceptid IS NOT NULL )
ORDER BY l.id, l.glossaryid, l.conceptid , f.userid";
    ...
    protected function find_new_categories_subscriptions($timenow)
    {
        ...
        $row_id = $DB->sql_concat_join("'_'", ['l.id', 'f.userid']);
        $sql = "
SELECT $row_id AS i, l.id AS logid, f.userid
FROM {block_glsubs_event_subs_log} AS l 
JOIN {block_glsubs_glossaries_subs} AS f ON f.glossaryid = l.glossaryid AND f.active = 0 AND f.newcategories = 1
WHERE l.processed = 0 AND l.categoryid > 0 AND l.timecreated < :timenow  AND l.eventtype = :eventtype AND ( l.categoryid IS NOT NULL ) 
ORDER BY l.id, l.glossaryid, l.categoryid , f.userid";
    ...
    protected function find_author_subscriptions($timenow)
    {
        ...
        $row_id = $DB->sql_concat_join("'_'", ['l.id', 'f.userid']);
        $sql = "SELECT $row_id AS i , l.id AS logid , a.userid 
FROM {block_glsubs_event_subs_log} AS l
JOIN {block_glsubs_glossaries_subs} AS f ON f.glossaryid = l.glossaryid AND f.active = 0 
JOIN {block_glsubs_authors_subs} AS a ON a.glossaryid = l.glossaryid AND a.authorid = l.authorid 
WHERE l.processed = 0 AND l.authorid > 0 AND l.timecreated < :timenow 
GROUP BY l.categoryid, l.conceptid ORDER BY i";
        try {
    ...
    protected function find_category_subscriptions($timenow)
    {
        ...
        // cater for a lot of user IDs to get unique query IDs
        $row_id = $DB->sql_concat_join("'_'", ['l.id', 'f.userid']);
        $sql = "SELECT $row_id AS i , l.id AS logid , c.userid , l.categoryid , l.conceptid, l.eventtext 
FROM {block_glsubs_event_subs_log} AS l
JOIN {block_glsubs_glossaries_subs} f ON f.userid = l.userid AND f.active = 0
JOIN {block_glsubs_categories_subs} c ON c.glossaryid = l.glossaryid AND c.categoryid = l.categoryid
WHERE l.processed = 0 AND l.authorid > 0 AND l.categoryid > 0 AND l.timecreated < :timenow
GROUP BY c.userid , l.categoryid, l.conceptid ORDER BY i";
        try {
    ...
    protected function find_concept_subscriptions($timenow)
    {
        ...
        // cater for a lot of user IDs to get unique query IDs
        $row_id = $DB->sql_concat_join("'_'", ['l.id', 'f.userid']);
        $sql = " SELECT $row_id AS i , l.id AS logid , c.userid , c.conceptactive , c.commentsactive
FROM {block_glsubs_event_subs_log} AS l
JOIN {block_glsubs_glossaries_subs} AS f ON f.userid = l.userid AND f.active = 0
JOIN {block_glsubs_concept_subs} AS c ON c.glossaryid = l.glossaryid 
    AND c.conceptid = l.conceptid 
    AND (c.conceptactive = 1 OR c.commentsactive = 1)
WHERE l.processed = 0 AND l.authorid > 0 AND l.conceptid > 0 AND l.timecreated < :timenow
GROUP BY c.userid , l.categoryid , l.conceptid ORDER BY i";
        try {
    ...
    protected function remove_deleted_concepts_subscriptions($timenow)
    {
        ...
        $sql = ' SELECT DISTINCT t.conceptid FROM {block_glsubs_concept_subs} AS t 
JOIN {block_glsubs_event_subs_log} AS l ON t.conceptid = l.conceptid AND l.processed = 0 AND l.timecreated < :timenow 
WHERE t.conceptid > 0 AND t.conceptid NOT IN (SELECT id FROM {glossary_entries})';
        try {
    ...
    protected function remove_deleted_category_subscriptions($timenow)
    {
        ...
        $sql = 'SELECT DISTINCT t.categoryid FROM {block_glsubs_categories_subs} AS t 
JOIN {block_glsubs_event_subs_log} AS l ON t.categoryid = l.categoryid AND l.processed = 0 AND l.timecreated < :timenow 
WHERE t.categoryid > 0 AND t.categoryid NOT IN (SELECT id FROM {glossary_categories})';
        try {
    ...
    protected function remove_deleted_author_subscriptions($timenow)
    {
        ...
        $sql = 'SELECT l.authorid FROM {block_glsubs_event_subs_log} AS l 
JOIN {user} AS u ON u.id = l.authorid and u.deleted = 1 
WHERE l.authorid > 0 AND l.processed = 0 AND l.timecreated < :timenow GROUP BY l.authorid ';
        try {
    ...
    protected function remove_deleted_concept_subscriptions($timenow)
    {
        ...
        // get the set of the latest erased glossary author IDs
        $sql = ' SELECT DISTINCT l.conceptid FROM {block_glsubs_event_subs_log} AS l
WHERE l.conceptid > 0 AND l.processed = 0 AND l.timecreated < :timenow AND l.conceptid NOT IN ( SELECT id FROM {glossary_entries} )
